    def compute_reward(self):
        ...
        # Updated weights for reward components
        velocity_weight = 1.0
        z_pos_weight = 0.5
        ...
        dof_pos_error = torch.relu(torch.abs(env.dof_pos - env.default_dof_pos) - (env.dof_pos_limits[:, 1] - env.dof_pos_limits[:, 0]).unsqueeze(0)/2)
        dof_limit_penalty = torch.sum(dof_pos_error, dim=1)
        
        action_smoothness = torch.mean(torch.abs(env.actions - env.last_actions), dim=1)
        
        # Reward components
        velocity_score = velocity_weight * (1 - torch.tanh(10 * velocity_error))
        z_pos_score = z_pos_weight * (1 - torch.tanh(10 * z_pos_error))
