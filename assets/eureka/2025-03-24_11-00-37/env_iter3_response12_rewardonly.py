    def compute_reward(self):
        ...
        # Adjusted weights and scaling factors
        velocity_weight = 1.0
        z_pos_weight = 0.5
        ...
        action_smoothness = torch.sqrt(torch.mean(torch.square(env.actions - env.last_actions), dim=1))
    
        # Scoring
        velocity_score = velocity_weight * (1 - torch.tanh(velocity_error))
        z_pos_score = z_pos_weight * (1 - torch.tanh(z_pos_error * 10))  # Increase the sensitivity
        orientation_score = orientation_weight * (1 - torch.tanh(orientation_error * 10))  # Increase the sensitivity
