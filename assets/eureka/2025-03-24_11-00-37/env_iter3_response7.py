...
class EurekaReward():
    ...
    def compute_reward(self):
        ...
        # Calculations
        velocity_error = torch.square(env.root_states[:, 7] - desired_velocity_x)
        z_pos_error = torch.square(env.root_states[:, 2] - desired_z_pos)
        ...
        # Scoring
        velocity_score = velocity_weight * (1 - torch.exp(-velocity_error))
        z_pos_score = z_pos_weight * (1 - torch.exp(-z_pos_error))
        orientation_score = orientation_weight * (1 - torch.exp(-orientation_error))
