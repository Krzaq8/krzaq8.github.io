    def compute_reward(self):
        ...
        # Updated weights for balance
        velocity_weight = 0.5
        z_pos_weight = 0.2
        orientation_weight = 0.05  # Reduced to diminish dominance
        dof_limit_penalty_weight = 0.005  # Slightly softer
        smoothness_weight = 0.002  # Significantly softer to encourage exploration and smoother transitions
    
        # Objectives
        desired_velocity_x = 2.0
        desired_z_pos = 0.34
        desired_orientation = torch.tensor([0., 0., 0., 1.], device=env.device)  # Quaternion for upright orientation
